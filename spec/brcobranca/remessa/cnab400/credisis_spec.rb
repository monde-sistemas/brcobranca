...
RSpec.describe Brcobranca::Remessa::Cnab400::Credisis do
  ...
  context 'validações dos campos' do
    ...
    context '@conta_corrente' do
      ...
      it 'deve ser inválido se a conta corrente tiver mais de 7 dígitos' do
        credisis.conta_corrente = '12345678'
        expect(credisis.invalid?).to be true
        expect(credisis.errors.full_messages).to include('Conta corrente deve ter 7 dígitos.')
      end
    end
  ...
  context 'formatacoes dos valores' do
    ...
    it 'info_conta deve retornar com 10 posicoes as informacoes da conta' do
      ...
      expect(info_conta.size).to eq 11
  ...
  context 'monta remessa' do
    ...
    context 'header' do
      it 'deve ter 39 posicoes' do
        expect(credisis.monta_header.size).to eq 39
      end
      ...
      it 'informacoes devem estar posicionadas corretamente no header' do
        ...
        expect(header[9..18]).to eq credisis.info_conta         # informacoes da conta
    ...
    context 'detalhe' do
      it 'deve ter 400 posições' do
        expect(credisis.monta_detalhe(pagamento, 1).size).to eq 400
      end
      ...
      it 'informacoes devem estar posicionadas corretamente no detalhe' do
        ...
        expect(detalhe[2..11]).to eq credisis.info_conta        # info da conta
    ...
    it 'remessa deve conter os registros mais as quebras de linha' do
      remessa = credisis.gera_arquivo
      expect(remessa.size).to eq 443

      # registros
      expect(remessa[0..38]).to eq credisis.monta_header
      expect(remessa[41..440]).to eq credisis.monta_detalhe(pagamento, 2).upcase
    ...
    context 'arquivo' do
      ...
      it { expect(credisis.gera_arquivo).to eq(read_remessa('remessa-credisis-cnab400.rem', banco_brasilia.gera_arquivo)) }
    end
  end
end
