...
module Brcobranca
  module Boleto
    class BancoBrasil < Base # Banco do Brasil
      ...
      validates_each :numero do |record, attr, value|
      ...
      def numero
        ...
        quantidade ? @numero.to_s.rjust(quantidade, '0') : @numero
      end
      ...
      def nosso_numero_dv
        "#{convenio}#{numero}".modulo11(mapeamento: { 10 => 'X' })
      end
      ...
      def nosso_numero_boleto
        digito = "-#{nosso_numero_dv}" unless @convenio.to_s.size == 7
        "#{convenio}#{numero}#{digito}"
      end
      ...
      def codigo_barras_segunda_parte
        ...
        when 8 # Nosso Número de 17 dígitos com Convenio de 8 dígitos e numero de 9 dígitos
          "000000#{convenio}#{numero}#{carteira}"
        when 7 # Nosso Número de 17 dígitos com Convenio de 7 dígitos e numero de 10 dígitos
          "000000#{convenio}#{numero}#{carteira}"
        when 6 # Convenio de 6 dígitos
          if codigo_servico == false
            # Nosso Número de 11 dígitos com Convenio de 6 dígitos e numero de 5 dígitos
            "#{convenio}#{numero}#{agencia}#{conta_corrente}#{carteira}"
          else
            # Nosso Número de 17 dígitos com Convenio de 6 dígitos e sem numero, carteira 16 e 18
        ...
        when 4 # Nosso Número de 7 dígitos com Convenio de 4 dígitos e sem numero
          "#{convenio}#{numero}#{agencia}#{conta_corrente}#{carteira}"
        end
      end
    end
